...
double rSphere = 20.0;                      // r, sphere radius, nm
double lMeshSide = 2.0;                       // l, triangular side length, nm
double c0_insertion = -0.1;                // spontaneous curvature of insertion
double c0_membrane = 0.0;                   // spontaneous curvature of membrane
double ds_insertion = 0.0;                       // insertion area ??UNUSED??
double ci_area_constraint = 1.0;       // area constraint coefficient ??UNUSED??
double miu_volume_constraint = 1.0;  // volume constraint coefficient ??UNUSED??
double kc = 20 * 4.17;                   // pN.nm  ??membrane bending constant
...
int main() {
	 ...
	 * Read parameter file
	 */
	//std::ifstream paramFile; paramFile.open("model.params");
	...
	setsphere_Loop_scheme(vertex, face, meanL, rSphere, lMeshSide);
	///////////////////////////////////////
	...
	for (int i = 0; i < numface; i++) {
		outfile << face(i, 0) + 1 << ',' << face(i, 1) + 1 << ','
				<< face(i, 2) + 1 << '\n';
	}
	outfile.close();
	ofstream outfile1("vertex_begin.csv");
	for (int i = 0; i < numvertex; i++) {
		outfile1 << setprecision(16) << vertex(i, 0) + 0.0 << ','
				<< vertex(i, 1) + 0.0 << ',' << vertex(i, 2) + 0.0 << '\n';
	}
	outfile1.close();
	///////////////////////////////////////////////////////
	Mat<int> insertionpatch;
	insertionpatch << 3029 << 2258 << 3017 << 579 << endr; // R=20 nm, no -1
	Row<int> Isinsertionpatch = determine_isinsertionpatch(face,
			insertionpatch);
	...
	//////////////////////////////////////////////////////////
	rowvec elementS(face.n_rows);
	elementS.fill(0);
	rowvec elementV(face.n_rows);
	elementV.fill(0);
	cell_area_volume(vertex, face, face_ring_vertex, GaussQuadratureN, elementS,
			elementV, gqcoeff, shape_functions); // calculate the elemental area and volume
	...
	param.k = k;
	param.K = K;
	param.c0_insertion = c0_insertion;
	param.c0_membrane = c0_membrane;
	param.meanL = meanL;
	param.gama_shape = gama_shape;
	param.gama_area = gama_area;
	param.sigma = sigma;
	param.GaussQuadratureN = GaussQuadratureN;
	...
	///////////////////////////////////////
	Row<double> energy(6);
	energy.fill(0.0); // E_bending, E_constraint, E_memvolume, E_regularization, E_insert, E_tot
	mat force(numvertex, 3);
	force.fill(0);
	...
	Energy_and_Force(vertex, vertexref, vertexi_nearby, vertexi_face, face,
			face_ring_vertex, param, spontcurv, insertionpatch,
			Isinsertionpatch, energy, force, deformnumbers, gqcoeff,
			shape_functions);

	mat Energy(Num_iteration, 6);
	Energy.fill(0);
	Energy.row(0) = energy;
	vec forcescale = force_scale(force);
	vec MeanForce(Num_iteration);
	MeanForce.fill(0);
	MeanForce(0) = mean(forcescale);
	vec totalarea(Num_iteration);
	totalarea(0) = param.S;
	vec totalvolume(Num_iteration);
	totalvolume(0) = param.V;
	...
	while (isCriteriaSatisfied == false && i < Num_iteration - 1) {
		// updates 
		if (i == 1 || i % 50 == 0) {
			a0 = lMeshSide / max(force_scale(s0)); //a0 = 1; 
		} else {
			a0 = a0 * 2e1;
		}
		{
			a1 = line_search_a_to_minimize_energy(a0, s0, vertex0, vertexref,
					vertexi_nearby, vertexi_face, face, face_ring_vertex, param,
					spontcurv, insertionpatch, Isinsertionpatch, isNCGstucked,
					energy0, force0, gqcoeff, shape_functions);
			vertex1 = update_vertex(vertex0, a1, s0, insertionpatch, face,
					param);
			if (a1 == -1) {
				cout << "step: " << i
						<< ". Note: no efficent step size a is found. Stop now! "
						<< endl;
			...
			Energy_and_Force(vertex1, vertexref, vertexi_nearby, vertexi_face,
					face, face_ring_vertex, param, spontcurv, insertionpatch,
					Isinsertionpatch, energy1, force1, deformnumbers, gqcoeff,
					shape_functions);
			...
			if (updateReference == true) {
				if (abs(Energy(i, 5) - Energy(i - 1, 5)) < 1e-3
						|| abs(MeanForce(i) - MeanForce(i - 1)) < 1e-3) {
		...
		// output parameters     
		cout << "step: " << i << ". Sratio= " << totalarea(i) / S0
				<< ", Vratio= " << totalvolume(i) / V0 << ". Energy= "
				<< Energy(i, 5) << ". meanF= " << MeanForce(i) << ". a= " << a0
				<< endl;
		cout << "step: " << i << ". Deform number: Area = " << deformnumbers(0)
				<< ", Shape = " << deformnumbers(1) << ", nodeform = "
				<< deformnumbers(2) << endl;
		// check whether to stop. if the total energy is flat for 100 simulation steps, then stop
		if (i > 500 && abs((Energy(i, 5) - Energy(i - 500, 5)) / 500) < 1e-3
				&& MeanForce(i) < criterion_force) {
		...
		if (i % 50 == 0) {
			...
			for (int j = 0; j < vertex1.n_rows; j++) {
				outfile << vertex1(j, 0) + 0.0 << ',' << vertex1(j, 1) + 0.0
						<< ',' << vertex1(j, 2) + 0.0 << '\n';
			}
			outfile.close();
		}
		...
		for (int j = 0; j <= i; j++) {
			...
			outfile2 << Energy(j, 0) + 0.0 << ',' << Energy(j, 1) + 0.0 << ','
					<< Energy(j, 2) + 0.0 << ',' << Energy(j, 3) + 0.0 << ','
					<< Energy(j, 4) + 0.0 << ',' << Energy(j, 5) + 0.0 << ','
					<< Ere_vs_Etot << ',' << totalarea(j) / S0 << ','
					<< totalvolume(j) / V0 << ',' << MeanForce(j) << '\n';
		}
		outfile2.close();
	}
	...
	for (int j = 0; j < vertex0.n_rows; j++) {
		outfile3 << setprecision(16) << vertex0(j, 0) + 0.0 << ','
				<< vertex0(j, 1) + 0.0 << ',' << vertex0(j, 2) + 0.0 << '\n';
	}
	outfile3.close();
}
